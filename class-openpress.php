...
class OpenPress
{
     ...
     * @var	  string
     */
    protected $plugin_slug = 'openpress';
    ...
    protected $openpress_image_base = 'content/images/wordpress';
    ...
    private function __construct()
    {
        ...
        if (isset($_GET['openpressexport'])) {
            add_action('init', array($this, 'download_file'));
        }

        if (isset($_GET['openpressjsonexport'])) {
    ...
    /**
     * Adds the Export link to the OpenPress links
    ...
    {
        $actions['export'] = '<a href="' . admin_url('tools.php?page=openpress') . '">Export</a>';
        return $actions;
    }
    ...
    public function load_plugin_textdomain()
    {
        ...
        $locale = apply_filters('openpress', get_locale(), $domain);
    ...
    public function add_plugin_admin_menu()
    {
        $this->plugin_screen_hook_suffix = add_management_page(
            __('Export to OpenPress', $this->plugin_slug),
            __('Export to OpenPress', $this->plugin_slug),
            'read',
            $this->plugin_slug,
    ...
    private function prepare_garray_structure()
    {
        ...
        $this->garray['data']['navigation_menus'] = array();
    }
    ...
    private function populate_posts()
    {
        ...
        if ($posts->have_posts()) {
            while ($posts->have_posts()) {
                ...
                $corrected_post_content = str_replace(get_site_url() . '/wp-content/uploads', '/' . $this->openpress_image_base, $post_content);
                ...
                $this->garray['data']['posts'][] = array(
                    ...
                    'mobiledoc'         => '{"version":"0.3.1","atoms":[],"cards":[["html",{"html":"' . str_replace(
                        ...
                        json_encode($corrected_post_content)
    ...
    /**
     * Convert the WordPress user slug to the name that OpenPress can use
     * @param string $wp_role The WordPress role slug
     * @return string The OpenPress role name
     */
    private function _get_openpress_user_role($wp_role)
    {
        switch ($wp_role):
            case "administrator":
    ...
    private function populate_users()
    {
        ...
        foreach ($users as $user) {
            ...
            $this->garray['data']['users'][] = array(
                ...
                'roles' => [$this->_get_openpress_user_role($user->roles[0])]
            );
        }
    ...
    public function populate_data()
    {
        ...
        $this->populate_navigation_menus();
    }
     ...
     * import script on OpenPress's end. So in order to safeguard against that, I had to implement something like the
    ...
    /**
     * Helper function to map WordPress statuses to OpenPress statuses
     * @param  string $wp_status the WordPress status
     * @return string			the OpenPress status
     */
    ...
    /**
     * Remove extra backslashes and quotes added due to double encoding and replace WordPress uploads URL with OpenPress relative URL.
    ...
    public function download_file()
    {
        ...
        $gfiledir = $upload_dir['basedir'] . '/openpress-exports';
        if (! file_exists($gfiledir)) {
            wp_mkdir_p($gfiledir);
        }
        $filename = 'wp_openpress_export.json';

        if (! is_writable($gfiledir)) {
            wp_die("<p>Uploads directory is not writable, can't save the OpenPress json file :/</p><p>Generated by the OpenPress plugin version {$this->version}.</p>", 'Directory not writable');
        }
        ...
        // Generate a zip archive of images in a OpenPress compatible directory structure as well as the JSON file.
        ...
        $gziparchivename = 'wp_openpress_export.zip';
        ...
        foreach ($files as $name => $file) {
            ...
            if (! $file->isDir() && ! in_array($file->getFilename(), $gexclusions) && in_array($gextension, $gincludedextensions)) {
                ...
                // Add current file to archive in dedicated WordPress folder within a OpenPress compatible directory structure.
                $gziparchive->addFile($filePath, $this->openpress_image_base . '/' . $relativePath);
    ...
    public function getopenpressmigratorversion()
    {
        return $this->version;
    }

    /**
     * Populates navigation menus on the export object
     * @return void                     modifies in place
     */
    private function populate_navigation_menus()
    {
